...
public class DBPipelineTest extends Specification {
	...
	def "成交日期 < 指定日期,应该不能增加到数据库"() {
		given:
		Condition.dbDealDate = Date.parse("yyyy-MM-dd", "2017-02-28")
		expect:
		!dbp.canAdd(dd)
	}
	...
	def "成交日期 = 指定日期,且数据库存在,应该不能增加到数据库"() {
		given:
		Condition.dbDealDate = Date.parse("yyyy-MM-dd", "2017-02-26")
